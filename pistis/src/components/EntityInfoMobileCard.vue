  ...
  <v-card class="pa-3 elevation-2 mobile-card">
    ...
    <!--
      <div>
        ...
        <v-slider v-model="entity.reliabilityScore" :color="color" thumb-size="0" hide-details max="100" min="0" readonly
        :step="1" />
      </div>
      -->
    ...
    <v-expand-transition>
      <div v-if="expanded">
        ...
        <div v-if="entity.entityGroups && entity.entityGroups.length > 0">
          ...
          <v-list density="compact">
            <v-list-item
              ...
              :title="ge.group.name"
              @click="router.push(`/group/${ge.group.id}`)"
            />
          </v-list>
        </div>
      </div>
...
<script lang="ts" setup>
  ...
  const props = defineProps<{ entity: Entity | NewEntity, preview: boolean }>()
