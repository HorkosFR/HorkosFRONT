  ...
  <v-container class="pa-0" fluid>
    <v-row class="ma-0">
      ...
      <v-col class="pa-6 overflow-auto" cols="12" md="6">
        <FactInfoCard :fact="factPreview" :preview="true" />
      </v-col>
    </v-row>
  </v-container>
</template>
...
<script lang="ts" setup>
  ...
  const factPreview = computed<Fact>(() => ({
    id: 'preview',
    type: originalFact.value.type ?? FactType.FactMisc,
    startDate: new Date(originalFact.value.startDate),
    endDate: originalFact.value.endDate ? new Date(originalFact.value.endDate) : null,
    context: originalFact.value.context ?? 'Contexte du fait',
    content: originalFact.value.content ?? '',
    summary: originalFact.value.summary ?? 'Résumé du fait',
    verdict: originalFact.value.verdict ?? 'Verdict du fait',
    isGoodAction: originalFact.value.isGoodAction ?? true,
    statement: originalFact.value.statement ?? '',
    tabTags: originalFact.value.tabTags ?? [],
    title: originalFact.value.title ?? 'Titre du fait',
    tags: originalFact.value.tabTags ? originalFact.value.tabTags.join(';') : '',
    reliabilityScore: null,
    gravityScore: null,
    gravityVoteAmount: null,
    reliabilityVoteAmount: null,
    entityId: originalFact.value.entityId,
    entity: null,
    factChecks: [],
    factSources: [],
  }))
